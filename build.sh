...
env | grep GO
go build -ldflags='-s -w' -v -o $TRAVIS_BUILD_DIR/thanos-inspect cmd/thanos-inspect/main.go
...
env | grep GO
go build -ldflags='-s -w' -v -o $TRAVIS_BUILD_DIR/thanos-inspect.exe cmd/thanos-inspect/main.go
